//
//  Default_PW_ViewController.swift
...
class Default_PW_ViewController: BaseNavigationViewController, View {
    ...
    func bind(reactor: Default_PW_ViewReactor) {
        self.pwInputView.textView.rx.text.orEmpty
        ...
        self.pwInputView.confirmButton.rx.tap
            .map { [weak self] _ in self?.originalPasswordText }
            .filterNil()
            .map(Reactor.Action.requestAuthCode)
            .bind(to: reactor.action)
            .disposed(by: self.disposeBag)
        
        reactor.state.map { $0.authCode }
            .filterNil()
            .distinctUntilChanged()
            .map(reactor.reactorForResetPW)
            .subscribe(onNext: { [weak self] reactor in
                let viewController = Default_PWReset_ViewController()
...
extension Default_PW_ViewController: SRPClearTextViewDelegate {
